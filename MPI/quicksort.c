...
int main(int argc, char ** argv)
{
  ...
  if(rank == 0)
  {
    //start = MPI_Wtime(); //iniciar tiempo, descomentar para iniciar el de la 211
...
//0      1       2     3
    ...
    if( (rank / partner) % 2 == 0)
    {
      ...
      printf("Recursividad %d en proceso %d\n", recvSize, rank);
    ...
    else
    {
      ...
      printf("es recursivo %d en proceso %d\n", recvSize, rank);
      if(recvSize > 0)
      {
        free((void *) recvBuffer);
        recvBuffer = (int *) malloc(sizeof(int)*recvSize);
